...
TMP_ROOT="${TMPDIR:-/tmp}"
API_LOG="$TMP_ROOT/proveria-pdf-text-layer-api.log"
WORKER_LOG="$TMP_ROOT/proveria-pdf-text-layer-worker.log"
API_PID=""
WORKER_PID=""

log() {
  printf '[pdf-text-layer] %s\n' "$*"
}
...
log "running PDF text-layer smoke flow"
PROVERIA_SMOKE_API_URL="$API_URL" pnpm --filter @proveria/worker smoke:pdf-text-layer

log "PDF text-layer smoke passed"
